...
import { useItalianMovieCatalog } from "@/api/use-italian-catalog";
...
export const Route = createFileRoute("/film/genres/$genres_and_id")({
  validateSearch: (search: Record<string, unknown>) => {
    ...
    if (legacy !== undefined && yearMin === undefined && yearMax === undefined) {
      yearMin = legacy;
      yearMax = legacy;
    }
...
function MoviePerGenresPage() {
  ...
  const { italianIds, isLoading: italianCatalogLoading } =
    useItalianMovieCatalog(italianOnly);
  ...
  const heroMovie = italianOnly && italianIds
    ? data.results.find((m) => italianIds.has(m.id)) ?? data.results[0]
    : data.results[0];

  const navigateYearSearch = (yearMin: number | undefined, yearMax: number | undefined) => {
    navigate({
  ...
  return (
    <HydrationBoundary state={dehydrate(queryClient)}>
      <div className={`flex flex-col ${!heroMovie ? "pt-4 md:pt-6" : ""}`}>
